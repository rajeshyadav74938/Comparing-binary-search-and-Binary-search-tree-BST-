...
node* deleteNodeFromBST(node * root, int key) { 
    ...
    if (key > root -> value) root -> pRight = deleteNodeFromBST(root -> pRight, key); 
    else if (key < root -> value) root -> pLeft = deleteNodeFromBST(root -> pLeft, key); 
    else { 
